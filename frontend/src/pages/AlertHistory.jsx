...
export default function AlertHistory() {
  ...
  const fetchHistory = async () => {
    setLoading(true)
    if (!supabase || !apiKey) { setLoading(false); return }
    const { data, error } = await supabase
      .from('alerts').select('*').eq('api_key', apiKey)
  ...
  const clearHistory = async () => {
    if (!supabase || !confirm('Clear all alert history?')) return
        ...
        {['ALL', 'BUY', 'SELL'].map((s) => (
          <button key={s} onClick={() => setFilter({ ...filter, signal: s })}
            className={`px-4 py-2 rounded-lg text-sm font-medium ${filter.signal === s
              ...
              : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>{s}</button>
        ))}
              ...
              {filtered.map((a) => (
                <tr key={a.id} className="border-b border-gray-800/50 hover:bg-gray-800/50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-bold ${a.signal === 'BUY' ? 'bg-green-900 text-green-400' : 'bg-red-900 text-red-400'}`}>{a.signal}</span>
                  </td>
